...
/**
 * The cleaner treatment: every hard offset shadow removed, the ground flattened
 * to near-solid, and the press-down transform dropped. The 2px borders stay —
 * they were doing the structural work; the shadows were only doing noise.
 *
 * Selection now reads by fill alone, so the hue coding has to carry more.
 */

const P = {
  ground: '#2d7a5f', groundTop: '#33846a',
  headline: '#f7f3e8', sub: '#c6e0d3',
...
const FONTS = {
  Inter: {
    link: 'family=Inter:opsz,wght@14..32,400..800',
    stack: "Inter, system-ui, sans-serif",
    titleWeight: 800, titleTrack: '-0.032em',
  },
  Epilogue: {
    link: 'family=Epilogue:wght@400..800',
    stack: "Epilogue, system-ui, sans-serif",
    titleWeight: 800, titleTrack: '-0.026em',
  },
}

const taste = (f) => `<!doctype html>
<html>
<head>
  <meta charset="utf-8">
...
<helmet>
  ...
  <style>
    body { margin: 0; font-family: ${f.stack}; }
...
<div style="width: 375px; min-height: 960px; background: linear-gradient(180deg, ${P.groundTop} 0%, {{ground}} 46%); color: ${P.headline}; display: flex; flex-direction: column;">
  ...
  <div style="display: flex; flex-direction: column; gap: 20px; padding: 32px 20px 12px;">
    ...
    <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 14px;">
      <div style="display: flex; flex-direction: column; gap: 10px;">
        ...
        <h1 style="margin: 0; font-size: 32px; font-weight: ${f.titleWeight}; line-height: 1.04; letter-spacing: ${f.titleTrack}; text-wrap: pretty;">Clos des Briords</h1>
    ...
    <div style="display: flex; flex-direction: column; gap: 8px;">
      <sc-for list="{{rows}}" as="row" hint-placeholder-count="6">
        <div style="border-radius: 12px; border: 2px solid ${P.ink}; overflow: hidden; background: {{row.shellBg}};">
          ...
          <button onClick="{{row.toggle}}" style="width: 100%; min-height: 54px; display: flex; align-items: center; justify-content: space-between; gap: 10px; padding: 9px 14px; background: none; border: none; text-align: left; color: ${P.ink};">
          ...
          <sc-if value="{{row.showFree}}" hint-placeholder-val="{{true}}">
            <div style="display: flex; flex-direction: column; gap: 16px; padding: 0 14px 16px; color: ${P.ink};">
              <div style="display: flex; align-items: center; justify-content: space-between; gap: 12px;">
                ...
                <span style="display: flex; align-items: center; gap: 10px;">
                  ...
                  <span style="min-width: 60px; text-align: center; font-size: 32px; font-weight: ${f.titleWeight}; letter-spacing: ${f.titleTrack}; color: {{scoreColor}};">{{scoreText}}</span>
              ...
              <div style="display: flex; flex-direction: column; gap: 7px;">
                <span style="font-size: 13px; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; color: ${P.muted};">Drink it with</span>
                <input placeholder="Oysters, or anything salty" style="width: 100%; min-height: 48px; padding: 0 13px; border-radius: 9px; border: 2px solid ${P.hairline}; background: ${P.cardOpen}; color: ${P.ink}; outline: none;">
              </div>
              <div style="display: flex; flex-direction: column; gap: 7px;">
      ...
      return {
        ...
        options: (a.options ?? []).map((o, j) => {
          ...
          return {
            ...
            // With the shadow gone, selection reads by fill and a firmer
            // border — so unselected steps back to a hairline instead.
...
for (const [name, f] of Object.entries(FONTS)) {
  await writeFile(new URL(`./Clean${name}.dc.html`, import.meta.url), taste(f))
  console.log('wrote Clean' + name + '.dc.html')
}
